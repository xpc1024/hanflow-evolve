...
list_py_files() {
  ...
  case "$mode" in
    full)
      ...
      ;;
    diff)
      # cycle 改动的 .py：对比 cycle base（默认分支）到 HEAD（含已 commit 的 P6 改动）。
      # LOOP P6 是 commit-per-task，P7 跑 --diff 时改动已提交，故对比 base..HEAD 而非工作树。
      # base 优先 main (2026-07 后统一), 回退 master (历史), 都不存在回退 HEAD (worktree)。
      local base_ref="main"
